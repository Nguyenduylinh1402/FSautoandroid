...
class RanorexLibrary(object):
    ...
    def __init__(self):
        ...
        self.model = None
        Ranorex.Mouse.DefaultMoveTime = 50 # default 300 miliseconds
        Ranorex.Keyboard.DefaultKeyPressTime = 0 # default 100 miliseconds
        Ranorex.Delay.SpeedFactor = 0
        Ranorex.Adapter.DefaultSearchTimeout = 2000 # 1 second

        # the below global setting use for finding element based on imaging
        Ranorex.Imaging.FindOptions.Default.Similarity = 0.77 # default 100%
    ...
    def __return_type(cls, locator):
        ...
        supported_types = ['AbbrTag', 'AcronymTag', 'AddressTag', 'AreaTag',
                           ...
                           'Tree', 'TreeItem', 'Unknown','TabPage','ToolBar']
    ...
    # default create element of ranorex library
    def __create_element(self, locator):
        ...
        if not element_created:
            raise AssertionError("'{}' not found after {} tries".format(locator, tries))
    ...
    def rn_element_text_should_be_in_x_time(self, elementLocator=None, text='',duration=0):
        """ Get element text and then compare to Text in duration

        :param locator: 
        - xpath selector of element
        - expected text
        - duration: how long do you need expected text appear
        :returns: True/False
        """
    ...
    def rn_find_element_based_on_image(self, elementLocator, path_to_image_file):
        """ 
        This keyword will find element based on its image
        
        Return from keyword: True/False
        """

        if self.debug:
            log.debug("Capture Element and compare contain")
        # load image
        img = Ranorex.Imaging.Load(path_to_image_file)

        # load element image
        element = self.__create_element(elementLocator)
        ele_img = element.CaptureCompressedImage()


        # find image
        if Ranorex.Imaging.Contains(ele_img,img):
            return True
        return False

    def rn_hover_mouse(self, element_locator):
        """ Hover above selected element

        :param locator: xpath pointing to desired element
        :return: None
        """

        element = self.__create_element(element_locator)
        mouse = Ranorex.Mouse()
        mouse.MoveTo(element.Element)

    def rn_disable_mouse_and_keyboard(self,is_diable=False):
        if self.debug:
            log.debug("Disable mouse and keyboard before run")
        
        Ranorex.Keyboard.Enabled = is_diable
        Ranorex.Mouse.Enabled = is_diable


    def rn_element_text_should_be_replaced_enterkey_by_space_trim_and_wait_text_appear(self,element_locator='',expected_text='',duration=0):
        if self.debug:
            log.debug("Get element text, replace enter key by space and trim and wait for expected text appear")
        
        duration = float(duration)/1000
        start = time.time()
        while(time.time()-start<=duration):
            element_text = self.rn_get_element_attribute(element_locator, 'Text')
...
def main():
    ...
    # start log
    logger.info("INFO: Start Remote library for robot framework")

    # define arguments
    parser = ArgumentParser(prog="rxconnector", description="Remote ranorex library for robot framework")
    parser.add_argument("-i","--ip", required=False, dest="ip", default="127.0.0.1")
    parser.add_argument("-p", "--port", required=False, type=int, dest="port", default=8270)
